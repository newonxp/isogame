package pathfinding
{
	...
	public class JumpPointSearchWoDiagonals
	{
		private static const sides:Vector.<Point>=new <Point>[new Point(1, 1), new Point(1, -1), new Point(-1, 1), new Point(-1, -1), new Point(0, 1), new Point(0, -1), new Point(1, 0), new Point(-1, 0)];

		private var _world:Grid;
		private var _jumps:int;

		public function JumpPointSearchWoDiagonals(grid:Grid)
		{
			_world=grid;
		}
		...
		private function nodeNeighbours(current:Node, neighbours:Vector.<Node>):void
		{
			if (!current.parent)
			{
				for (var i:int=0; i < 8; i++)
				{
					...
					{
						if (current.x == potentialX || current.y == potentialY)
						{
							//trace(current.x + " " + current.y + " ++++ " + potentialX + " " + potentialY)
			...
			if (directionX != 0 && directionY != 0)
			{
				...
				if (_world.isBlocked(current.x - directionX, current.y) && !_world.isBlocked(current.x, current.y + directionY))
				{
					neighbours.push(Node.pooledNode(current.x - directionX, current.y + directionY));
				}
				if (_world.isBlocked(current.x, current.y - directionY) && !_world.isBlocked(current.x + directionX, current.y))
			...
			else
			{
				...
				{
					neighbours.push(Node.pooledNode(current.x, current.y + 1));
				}
				if (!_world.isBlocked(current.x, current.y - 1))
				{
					neighbours.push(Node.pooledNode(current.x, current.y - 1));
				...
				{
					neighbours.push(Node.pooledNode(current.x + 1, current.y));
				}
				if (!_world.isBlocked(current.x - 1, current.y))
				{
					neighbours.push(Node.pooledNode(current.x - 1, current.y));
		...
		private function jump(currentX:int, currentY:int, directionX:int, directionY:int, start:Node, end:Node, view:Boolean=false):Node
		{
			_jumps++;
			...
			/*	if (directionX != 0 && directionY != 0)

				{
					...
					while (true)
					{
						if (!_world.isBlocked(offsetX - directionX, offsetY + directionY) && _world.isBlocked(offsetX - directionX, offsetY) || !_world.isBlocked(offsetX + directionX, offsetY - directionY) && _world.isBlocked(offsetX, offsetY - directionY))
						{
						...
						if (jump(offsetX, offsetY, directionX, 0, start, end) != null || jump(offsetX, offsetY, 0, directionY, start, end) != null)
						{
							return Node.pooledNode(offsetX, offsetY);
						}
			...
			if (directionX != 0)
			{
				while (true)
				{
					if (!_world.isBlocked(offsetX + directionX, nextY + 1) && _world.isBlocked(offsetX, nextY + 1) || !_world.isBlocked(offsetX + directionX, nextY - 1) && _world.isBlocked(offsetX, nextY - 1))
					{
						return Node.pooledNode(offsetX, nextY);
					}
					if (view == false)
					{
						if (jump(offsetX, offsetY, 0, 1, start, end, true) != null || jump(offsetX, offsetY, 0, -1, start, end, true) != null)
						{
							return Node.pooledNode(offsetX, offsetY);
						}
					}
			...
			else
			{
				while (true)
				{
					...
					if (!_world.isBlocked(nextX + 1, offsetY + directionY) && _world.isBlocked(nextX + 1, offsetY) || !_world.isBlocked(nextX - 1, offsetY + directionY) && _world.isBlocked(nextX - 1, offsetY))
					{
						return Node.pooledNode(nextX, offsetY);
					}
					if (view == false)
					{
						if (jump(offsetX, offsetY, 1, 0, start, end, true) != null || jump(offsetX, offsetY, -1, 0, start, end, true) != null)
						{
							return Node.pooledNode(offsetX, offsetY);
						}
					}
		...
		private function identifySuccessors(current:Node, start:Node, end:Node):Vector.<Node>
		{
			...
			for each (var neighbour:Node in neighbours)
			{
				...
				//trace(current.x + " " + current.y + " ++++ " + directionX + " " + directionY)
		...
		public function findPath(start:Point, end:Point):Path
		{
			...
			if (!closedSet)
				closedSet=new IntHashSet(512 * 4);
			else
				closedSet.clear(false);

			if (!openSet)
				openSet=new PriorityQueue(true, 512 * 4);
			else
				openSet.clear(false);
